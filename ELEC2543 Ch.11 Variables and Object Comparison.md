...
>
> Java 严格区分基本类型与对象引用的比较语义：`==` 对基本类型比较值、对对象引用比较地址，而内容等价性的判断依赖 `equals` 方法的定义与覆写 (*Overriding*)。这一取向要求程序员在自定义类中显式声明“相等”的含义，而非默认沿用语言内建行为，与 C++ 通过重载 `operator==` 隐式引入结构相等、或 Python 通过 `__eq__` 与 `__hash__` 定义对象等价的方式形成对照。
>
